...
custom_intro = "Ты — AI-ассистент. Ответь на вопрос, используя контекст ниже. Перепроверь корректность информации."
...
def generate_response(prompt: str, context: str, web_context: str = "") -> str:
    ...
    try:
        ...
        return answer if answer else "⚠️ Ответ не получен."

    except Exception as e:
        if DEBUG_MODE:
            print("Ошибка генерации ответа:")
            traceback.print_exc()
        return "⚠️ Произошла ошибка при генерации ответа. Попробуйте позже."

def update_custom_intro(message):
    global custom_intro
    custom_intro = message.text.strip()
    bot.send_message(message.chat.id, f"✅ Промт будет обновлен:\n\n{custom_intro}")
    if DEBUG_MODE:
        print(f"[PROMPT] Обновлена первая строка промта: {custom_intro}")
...
def create_main_keyboard():
    ...
    btn_prompt = types.KeyboardButton("✏️ Изменить промт")
    ...
    if DEBUG_MODE:
        ...
        markup.add(btn1, btn2, btn3, btn4, btn5, btn6, btn7, btn_prompt)
    else:
        markup.add(btn1, btn2, btn_prompt, btn3)
...
def handle_files(message):
    ...
    if message.document:
        ...
        elif message.text == "✏️ Изменить промт":
            bot.send_message(message.chat.id, "Введите новый шаблон промта.")
            bot.register_next_step_handler(message, update_custom_intro)
